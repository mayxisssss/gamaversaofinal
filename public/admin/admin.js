...
(function(){
  ...
  function input(n,l,t='',v=''){
    const isToggle = n === 'enabled' || n === 'showLaunchBadge' || ['quickHighlights','dynamicSections','newsPreview','videosPreview','newsPage','videosPage','galleryPage','eventsPage','floatingSocials'].includes(n);
    if(isToggle){
      const val = String(v ?? '').toLowerCase();
      const off = v === false || ['false','não','nao','0','off','desativado'].includes(val);
      return `<div class="form-row"><label>${l}</label><select name="${n}"><option value="sim" ${!off?'selected':''}>Sim / Ativado</option><option value="não" ${off?'selected':''}>Não / Desativado</option></select></div>`;
    }
    if(t==='textarea')return`<div class="form-row full"><label>${l}</label><textarea name="${n}">${esc(v)}</textarea></div>`;
    if(t==='image')return`<div class="form-row full"><label>${l}</label><input class="input" name="${n}" value="${esc(v)}" placeholder="URL ou upload"><input class="input" data-file="${n}" type="file" accept="image/*" style="margin-top:8px"></div>`;
    return`<div class="form-row"><label>${l}</label><input class="input" name="${n}" value="${esc(v)}"></div>`;
  }
  ...
  function read(form,fields){const fd=new FormData(form);const o={};fields.forEach(([n])=>{
    const isToggle = n === 'enabled' || n === 'showLaunchBadge' || ['quickHighlights','dynamicSections','newsPreview','videosPreview','newsPage','videosPage','galleryPage','eventsPage','floatingSocials'].includes(n);
    if(isToggle){ const raw = String(fd.get(n)||'sim').toLowerCase(); o[n] = !(raw === 'não' || raw === 'nao' || raw === 'false' || raw === '0' || raw === 'off'); }
    else o[n]=fd.get(n)||'';
  });return o}
