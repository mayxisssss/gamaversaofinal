...
(function () {
  ...
  function isEnabled(value) {
    ...
    if (["false", "não", "nao", "0", "off", "desativado", "desativada", "hidden", "oculto"].includes(normalized)) return false;
    return true;
  }
  ...
  function renderHeader() {
    ...
    const contentMenu = visibleContentItems.length ? `<div class="content-nav-group ${visibleContentItems.some(([, , page]) => currentPage === page) ? "is-active" : ""}">
      <button class="content-nav-button" type="button" data-content-nav-toggle aria-expanded="false">Conteúdos <span class="content-chevron">▾</span></button>
      <div class="content-nav-dropdown" aria-label="Submenu de conteúdos">
    ...
    mount.innerHTML = `
      <header class="site-header" data-header>
        <div class="container header-inner">
          ...
          <nav class="main-nav" aria-label="Menu principal" data-main-nav>${searchBox}${links}${contentMenu}</nav>
  ...
  function initMenu() {
    ...
    if (!window.__gamaContentMenuCloseBound) {
      document.addEventListener("click", (event) => {
      ...
      window.__gamaContentMenuCloseBound = true;
    }
  }
